	...
	$.Pagination.prototype = {
		...
		_layout : function() {
			...
			if(this.isSliderOpened ){
				this.slider.getSliderEl().addClass( 'sp-slider-open' );
				this.$el.stop().applyStyle( { width : this.options.width }, $.extend( true, [], { duration : '150ms' } ) );
			}
			if(this.options.isHelp){
				var self = this;
				var times = 0;
				var timer = setInterval(function () {
					if(times<self.options.total){
						self.slider.next( function( value ) {
							self.options.onChange(value);
						})
					}else if(times<self.options.total*2){
		...
		_initEvents : function() {
			...
			this.slider.getSlider().on( {
				...
				//
				// 	};
				// 	self.$el.stop().applyStyle( { width : 0 }, $.extend( true, [], { duration : '1500ms', complete : animcomplete } ) ).on( self.transEndEventName, function() {
				//
